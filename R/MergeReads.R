...
#'@param pctID Minimum % identity of alignment between forward and reverse read. Decrease with greater overlap of forward and reverse sequences.
...
MergeReads <- function(maxdiffs=15,pctID=80,folderwfiles="1.rawreads",folderoutput="2.mergedreads",usearchdest="usearch"){
  ...
  for (file in files){
    ...
    gsub(folderwfiles,folderoutput,file)
    output <- gsub('(^.*/.*)_S[0-9]+_L001_R1_001.fastq','\\1.merged.fastq',gsub(folderwfiles,folderoutput,file))
    mergearg <- paste("-fastq_mergepairs",file,"-fastq_maxdiffs",maxdiffs,"-fastq_pctid",pctID,"-fastqout",output)
