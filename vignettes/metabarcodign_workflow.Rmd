...
There are a great number of pipelines and methods for the analysis of metabarcoding data. No single method is appropriate or inappropriate in all situations. Methods and ideas are constantly being developed so keep an eye on the horizon and focus on fundemental and transferable skills. 
...
Great, lets check the folders. We can use `dir()` as before. We should see 8 folders with different names prefixxed with a number.
...
First lets check our files to make sure everyhting is as we expect.
```{r}
list.files("1.rawreads/")
```
We should have pairs of gun zipped samples ready for analysis. 


Now everything is in place we can start to analyse the data. First lets unzip all the data, and then check to make sure everyhting has been unzipped. 
```{r}
Unzip()
list.files("1.rawreads/")
```
...
If we prefer we can perform the same operation using the metadata sheet that we have copied into the base of the working directory. 
...
We can calcuate the mean and standard deviation of reads across the raw samples.
...
PrimerStrip(PrimerF = "NNNNNNGGWACWGGWTGAACWGTWTAYCCYCC",
            ...
            MinLen = 303,
            MaxLen = 323,
            cutadaptdest = "/cutadapt/location/here",
            ncores=7)
```
...
Next we pool all our sequences together and disgard singleton sequences (sequences only found once in the data). 
...
```{r}
OTUCluster(usearchdest = "/path/to/useach")
```
...
```{r}
DenoiseUNOISE3(usearchdest = "/path/to/useach")
```
